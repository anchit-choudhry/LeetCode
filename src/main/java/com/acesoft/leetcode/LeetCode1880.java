...
public class LeetCode1880 {
    ...
    public boolean isSumEqual(String firstWord, String secondWord, String targetWord) {
        ...
        for (i = firstWord.length() - 1; i >= 0; i--) {
            firstNum += Math.pow(10, j++) * Integer.parseInt(String.valueOf(firstWord.charAt(i) - 'a'));
        }
        j = 0;
        for (i = secondWord.length() - 1; i >= 0; i--) {
            secondNum += Math.pow(10, j++) * Integer.parseInt(String.valueOf(secondWord.charAt(i) - 'a'));
        }
        j = 0;
        for (i = targetWord.length() - 1; i >= 0; i--) {
            targetNum += Math.pow(10, j++) * Integer.parseInt(String.valueOf(targetWord.charAt(i) - 'a'));
        }
